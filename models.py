...
# --- COMPATIBILITY PATCHES (MUST BE BEFORE GLUONTS IMPORTS) ---
try:
    import gluonts.torch.util
    
    # 1. Define the fixed function (converting list to tuple)
    def fixed_slice_along_dim(a, dim, slice_):
        # This mirrors the original logic
        idx = [slice(None)] * len(a.shape)
        idx[dim] = slice_
        # THE FIX: Convert the list 'idx' to a tuple before passing to PyTorch
        return a[tuple(idx)]

    # 2. Apply the monkey patch
    gluonts.torch.util.slice_along_dim = fixed_slice_along_dim
    
    # Optional: If you still see warnings about 'take_slice', keep this too:
